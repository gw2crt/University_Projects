...
function generateNumber() {  //Function which generates the random Numbers
    ...
    var circleR = 146;      // variable storing the radius size of the circle     
    var colour = [ '#F6A329' ];   //variable array to store the colours used within the large ball. 
    ...
    counter++; // simple counter used to display the number of balls which have been drawn. 
        ...
        alert("End of Game"); //message to end the game once the condition is met and 90 balls have been generated.
    }
...
function drawLargeBall(ctx,x,y,r,colour) { //function to draw the large bing ball with random numbers, variables have been passed from the generate number function
    ctx.beginPath();                  //built in function to begin the path of the circle
...
function restart(ctx) {
    ...
    calledNumbers.length = 0;
                                                                                                        //used to restart the games by removing any array objects.
        ...
        while (randomNum.length < 90){                                                      //while condition to loop to generate 90 unique numbers. 
            ...
            for(var i=0;i<randomNum.length; i++){                                           //for loop checking against the array length this will loop until he condition is met
                ...
                if(randomNum[i]==ranNum){                                                      // An if statement to check if the number generated in the array already exists.
                    found = true; break}                                                                // a little trick to flip the value of a boolean variable if a reapeat number is generated starting the loop again. 
                }
...
function conversionDay(dn ){
    var dayNames = new Array("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday");                 // function to generate and hold the array for the time and date returning the day                                                                                                                                                                                                         names as a string.
    
    return dayNames[dn];                                                                                                                                                             //used to return the string stored within the array to the dayName variable. 
}

function conversionMonth(mn) {    
    var monthNames = new Array("Jan", "Feb", "Mar", "Apr", "May", "June", "July", "Aug", "Sept", "Oct", "Nov", "Dec");          // function to generate and hold the arrays for the time and date returning the day                                                                                                                                                                                                       names as a string.
...
function drawMatrix() {
    ...
    rad =  42;                                                                                                                                                                                    // The radius of the centre of the circle in pixels
    var gapBetweenCircles = 10;                                                                                                                                                     // The gap between the bingo Numbers which have been drawn
    ...
    var transparency = "rgba(255, 255, 255, 0.2)";                                                                                                                                                                                 
    matrixNumber = [];
    matrixCounter = 1;
    ...
    for (var j = 1; j <= 9; j++){                                                                                                                                                           // Nested for Loops to created the 10 x 9 bingo ball matrix
        for(var i = 1; i <= 10; i++){                                                                                                                                                      // The second part of the nested for Loop which is run 10 times and then passed to                                                                                                                                                                                               the outer loop to create a new row
            ...
            if (ranNum == matrixCounter){  //conditions to meet if the balls have been called which change the ball colour respectivly  
                ...
                ctx.fillStyle = "#FF1909";        
            }else {
                ctx.fillStyle = "#70D9F6";
            }
            ctx.fill();
            ctx.lineWidth = 1;
